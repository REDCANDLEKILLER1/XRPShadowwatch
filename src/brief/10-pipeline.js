...
// The prayer and scripture are generated by buildDailyTone(pack) in 02-core,
// which returns { tone, headline, scripture:{ref,text}, prayer } off a
// date-seeded rotation. The pipeline was looking for getDailyPrayer /
// buildDailyPrayer / getDailyScripture / buildDailyScripture — none of which
// exist — so it always fell through to "[Today's prayer — see dev panel]" while
// the real text sat in the structured report. Read the actual engine, preferring
// state.dailyTone when the scan already computed it.
function _dailyTone(pack){
  try{
    if(typeof state!=='undefined' && state && state.dailyTone) return state.dailyTone;
  }catch(_){}
  if(typeof buildDailyTone==='function')
    return _safe(function(){ return buildDailyTone(pack||{}); }, null);
  if(typeof window.buildDailyTone==='function')
    return _safe(function(){ return window.buildDailyTone(pack||{}); }, null);
  return null;
}

function _buildPrayer(pack){
  var tone=_dailyTone(pack);
  if(tone && typeof tone.prayer==='string' && tone.prayer.trim()) return tone.prayer.trim();
...
function _buildScripture(pack){
  var tone=_dailyTone(pack);
  if(tone && tone.scripture){
    var ref=String(tone.scripture.ref||'').trim();
    var txt=String(tone.scripture.text||'').trim();
    // Same shape the structured report publishes: reference line, then the verse
    // in quotes. Never emit a bare quote with no reference behind it.
    if(ref && txt) return ref + '\n\u201C' + txt + '\u201D';
    if(txt) return '\u201C' + txt + '\u201D';
  }
