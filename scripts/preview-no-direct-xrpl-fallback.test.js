...
(async () => {
  ...
  // Browser behavior lives in hotfix 47 so 02-core.js remains frozen on PR #83.
  // The fence intercepts only the named preview refusal and turns it into an
  // explicit incomplete evidence result before the legacy catch can authorize
  // a direct account_tx fallback.
  const fence = fs.readFileSync(path.join(__dirname, '../src/brief/47-preview-no-direct-xrpl-20260923.js'), 'utf8');
  assert(fence.includes("[preview-fence] installed"), 'preview fence install marker missing');
  assert(fence.includes("Preview read-only snapshot unavailable — direct XRPL fallback remains disabled."),
    'preview fail-closed log missing');
  assert(fence.includes("PREVIEW_READ_ONLY_SNAPSHOT_UNAVAILABLE|EVIDENCE_WRITE_REFUSED_NON_PRODUCTION"),
    'preview refusal classifier missing');
  assert(fence.includes("live_acquisition_disabled: true"), 'synthetic incomplete result must forbid live acquisition');
  assert(fence.includes("xrpl_requests: 0"), 'synthetic incomplete result must claim zero XRPL requests');
  assert(!fence.includes("account_tx"), 'hotfix must not implement or invoke direct account_tx');
